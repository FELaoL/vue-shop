...
function doGetConsignee(info) {
	return new Promise(function (resolve, reject) {
		if (!info.order) return reject("订单对象未创建");

		dao.list("ConsigneeModel", { columns: { order_id: info.order.order_id } }, function (err, consignee) {
			if (err) return reject("获取订单收货地址失败");

			info.order.consignee = consignee[0];
			resolve(info);
		});
	});
}

function doCreateOrUpdateConsignee(params) {
	return new Promise(function (resolve, reject) {
		dao.list("ConsigneeModel", { columns: { order_id: params.order_id } }, function (err, consignee) {
			if (err) return reject("获取订单收货地址失败");
			if (consignee && consignee.length > 0) {
				dao.update("ConsigneeModel", consignee[0].cgn_id, _.clone(params), function (err, consignee1) {
					if (err) return reject("更新订单收货地址失败");
					// info.order = newOrder;
					resolve(consignee1);
				});
			} else {
				dao.create("ConsigneeModel", params, function (err, consignee2) {
					if (err) return reject("创建订单收货地址失败");
					resolve(consignee2);
				});
			}
			// resolve(info);
...
module.exports.getAllOrders = function (params, cb) {
	...
	dao.countByConditions("OrderModel", conditions, function (err, count) {
		...
		dao.list("OrderModel", conditions, function (err, orders) {
			...
			resultDta["orders"] = _.map(orders, function (order) {
...
module.exports.updateOrder = function (orderId, params, cb) {
	...
	// 	cb(err);
	// });
	doCreateOrUpdateConsignee(params)
